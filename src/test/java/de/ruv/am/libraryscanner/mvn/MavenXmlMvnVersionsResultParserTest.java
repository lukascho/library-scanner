...
public class MavenXmlMvnVersionsResultParserTest {
    ...
    public void test_parseMavenDependencyReport() {
        ...
        final List<String> incrementals = mvnVersionsResult.getDependencies().get(1).getIncrementals();
        final List<String> minors = mvnVersionsResult.getDependencies().get(1).getMinors();
        final List<String> majors = mvnVersionsResult.getDependencies().get(1).getMajors();
        ...
        // incrementals
        assertEquals("3.0.1", incrementals.get(0));
        assertEquals("3.0.2", incrementals.get(1));
        assertEquals("3.0.3", incrementals.get(2));
        assertEquals("3.0.4", incrementals.get(3));
        assertEquals("3.0.5", incrementals.get(4));
        assertEquals("3.1.0-alpha-1", incrementals.get(5));

        // minors
        assertEquals("3.1.0", minors.get(0));
        assertEquals("3.1.1", minors.get(1));
        assertEquals("3.2.1", minors.get(2));

        // majors
        assertEquals("4.0.0", majors.get(0));
